...
import { updatePairDayData, updateTokenDayData, updateAlmanackDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let almanack = AlmanackFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  almanack.totalLiquidityBNB = almanack.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  almanack.totalLiquidityBNB = almanack.totalLiquidityBNB.plus(trackedLiquidityBNB);
  almanack.totalLiquidityUSD = almanack.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  almanack.totalTransactions = almanack.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let almanack = AlmanackFactory.load(FACTORY_ADDRESS);
  almanack.totalVolumeUSD = almanack.totalVolumeUSD.plus(trackedAmountUSD);
  almanack.totalVolumeBNB = almanack.totalVolumeBNB.plus(trackedAmountBNB);
  almanack.untrackedVolumeUSD = almanack.untrackedVolumeUSD.plus(derivedAmountUSD);
  almanack.totalTransactions = almanack.totalTransactions.plus(ONE_BI);
  ...
  let almanackDayData = updateAlmanackDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  almanackDayData.dailyVolumeUSD = almanackDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  almanackDayData.dailyVolumeBNB = almanackDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  almanackDayData.dailyVolumeUntracked = almanackDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  almanackDayData.save();
